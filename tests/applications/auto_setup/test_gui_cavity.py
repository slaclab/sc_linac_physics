...
def test_acon_edit_is_spinbox(gui_cavity):
    assert isinstance(gui_cavity.acon_edit, PyDMSpinbox)
...
def test_on_lock_clicked_when_locked_yes_unlocks(gui_cavity):
    gui_cavity.lock()
    with patch(
        "sc_linac_physics.applications.auto_setup.frontend.gui_cavity.make_sanity_check_popup"
    ) as mock_popup:
