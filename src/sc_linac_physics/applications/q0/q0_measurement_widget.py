...
class Q0MeasurementWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        main_layout.addWidget(
            self.groupBox_2, 4, 0, 1, 2
        )  # Row 4, span 2 columns

        # RF Measurement
        self.rf_groupbox = self._create_rf_measurement_group()
        main_layout.addWidget(
            self.rf_groupbox, 5, 0, 1, 2
        )  # Row 5, span 2 columns
    ...
    def _create_cryo_controls_group(self):
        ...
        # Restore Button
        self.restore_cryo_button = QPushButton(
            "Restore Cryo to Standard Values"
        )

        # Add sub groups and button to main Cryo Controls layout
        layout.addWidget(
            jt_groupbox, 0, 0, 1, 2
        )  # JT covers first 2 columns in row 0
    ...
    def _create_calibration_group(self):
        ...
        self.start_heat_spinbox.setRange(
            75.0, 130.0
        )  # Use setRange for min/max
        ...
        self.show_cal_data_button.setEnabled(
            False
        )  # Keep non default enabled state
