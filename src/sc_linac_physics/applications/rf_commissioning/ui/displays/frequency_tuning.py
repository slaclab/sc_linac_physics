...
class FrequencyTuningDisplay(BasePlaceholderDisplay):
    ...
    # Carries (detune_hz, live_steps_or_None) from the 500ms EPICS read thread.
    # Using a signal guarantees Qt dispatches the slot to the GUI thread;
    # QTimer.singleShot(0, lambda) without a QObject context runs in the
    # calling thread's event loop, which doesn't exist for daemon Thread()s.
    _live_detune_ready = pyqtSignal(float, object)
    ...
    def setup_ui(self) -> None:
        ...
        self._live_detune_ready.connect(self._apply_live_detune)
    ...
    def _on_tuning_data_point(self, signed_steps: float, detune: float) -> None:
        ...
        first_point = not self._live_steps
    ...
    def _fetch_and_update_live_detune(self) -> None:
        """Read live EPICS values off the GUI thread, then dispatch the UI update."""
        try:
    ...
    def _apply_live_detune(self, detune: float, live_steps) -> None:
        """Apply a live detune reading to the plot curves (runs on the GUI thread)."""
        steps = live_steps if live_steps is not None else self._live_steps[-1]
        if self._cursor_curve is not None:
            self._cursor_curve.setData([steps], [detune])
        # Only extend the live-curve tail while a stage is actively running.
        # After a stage completes the cavity may drift, and extending the tail
        # with post-completion readings would make the plot show drift as if it
        # were part of the tuning run.
        if self._live_curve is not None and self._controller._stage_running:
            self._live_curve.setData(
                self._live_steps + [steps],
                self._live_hz + [detune],
            )
        if (
            self._controller._stage_running
            and self._controller._probe_stage_confirmed
            and self._controller._current_stage >= 3
