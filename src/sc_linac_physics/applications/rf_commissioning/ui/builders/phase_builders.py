...
class PiezoPreRFUI(PhaseUIBase):
    ...
    def build(self) -> QHBoxLayout:
        ...
        left_panel.addWidget(self._build_history(), stretch=1)

        right_panel = QVBoxLayout()
        right_panel.setSpacing(6)

        right_panel.addWidget(
            self._build_combined_results_section("Piezo Pre-RF")
        )
        right_panel.addWidget(
...
class _StandardPlaceholderUI(PhaseUIBase):
    ...
    def build(self) -> QHBoxLayout:
        ...
        right_panel.setSpacing(6)
        right_panel.addWidget(
            self._build_basic_results_section(self.PHASE_TITLE)
        )
        right_panel.addWidget(
            self._build_stored_data_section(
                self._get_parent_stored_data_fields()
...
class SSACharUI(PhaseUIBase):
    ...
    def build(self) -> QHBoxLayout:
        ...
        right_panel = QVBoxLayout()
        right_panel.setSpacing(6)
        right_panel.addWidget(self._build_ssa_results())
        right_panel.addWidget(
            self._build_stored_data_section(
                self._get_parent_stored_data_fields()
            )
...
class FrequencyTuningUI(PhaseUIBase):
    ...
    def build(self) -> QVBoxLayout:
        ...
        right = QVBoxLayout()
        right.setSpacing(4)
        right.addWidget(self._build_history(), stretch=1)
        stored = self._build_stored_data_section(
            self._get_parent_stored_data_fields()
        )
        right.addWidget(stored)
    ...
    def _build_stage1_card(self) -> QFrame:
        return self._build_stage_card(
            ...
            description="Check that the stepper is ready, then record the cavity's resting frequency.",
    ...
    def _build_stage2_card(self) -> QFrame:
        return self._build_stage_card(
            stage=2,
            title="② Probe Stepper",
            description="Move the stepper a known distance to measure how much the frequency changes per step.",
    ...
    def _build_stage3_card(self) -> QFrame:
        return self._build_stage_card(
            ...
            description="Use the measured Hz/step to move the cavity to its resonant frequency.",
    ...
    def _build_stage4_card(self) -> QFrame:
        return self._build_stage_card(
            ...
            description="Scan the cavity to find the 8π/9 and 7π/9 parasitic mode frequencies.",
