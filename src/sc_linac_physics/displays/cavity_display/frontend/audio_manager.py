...
class AudioAlertManager(QObject):
    ...
    def __init__(self, gui_machine, parent=None):
        ...
        # Don't start automatically: self.escalation_timer.start(30000)

        # Don't connect to cavities yet - wait until enabled
    ...
    def acknowledge_cavity(self, cavity_id):
        ...
        # Also remove from alerted sets to allow re-alerting if needed later
        # (but only after acknowledgment expires or is cleared)
        # Commenting this out keeps it from re-beeping on same alarm
        # self.alerted_alarms.discard(cavity_id)
        # self.alerted_warnings.discard(cavity_id)
