...
class SRFHome(Display):
    def __init__(self):
        ...
        self.mini_home_groupbox.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Maximum)
        ...
        quench_reset_path = f"{self.root_dir}/applications/quench_processing/quench_resetter.py"
        cav_disp_backend_path = f"{self.root_dir}/displays/cavity_display/backend/runner.py"

        self.sel_phase_opt_groupbox = make_watcher_groupbox(watcher_name="SC_SEL_PHAS_OPT", script_path=sel_opt_path)
        self.quench_reset_groupbox = make_watcher_groupbox(
            watcher_name="SC_CAV_QNCH_RESET", script_path=quench_reset_path
        )
        self.cav_disp_groupbox = make_watcher_groupbox(watcher_name="SC_CAV_FAULT", script_path=cav_disp_backend_path)
    ...
    def fill_link_groupbox(self):
        ...
        unit_test_button = PyDMEDMDisplayButton(filename="$EDM/llrf/rf_srf_cavity_unit_all.edl")
        ...
        microphonics_button = PyDMRelatedDisplayButton(filename="/home/physics/srf/gitRepos/microphonics/CommMicro.py")
        microphonics_button.setText("Microphonics GUI")
        buttons.append(microphonics_button)

        for decarad in [1, 2]:
            decarad_button = PyDMRelatedDisplayButton(filename="$TOOLS/pydm/display/ads/decarad_main.ui")
    ...
    def add_buttons_from_path(self, buttons: List[PyDMRelatedDisplayButton], suffix):
        for file in pathlib.Path(self.root_dir).rglob(suffix):
            ...
            if name.startswith("test"):
                continue
            gui_button = PyDMRelatedDisplayButton(filename=os.path.join(self.root_dir, file))
    ...
    def fill_mini_home_groupbox(self):
        ...
        for linac in range(4):
            ...
            cryo_button = PyDMEDMDisplayButton(filename=f"$EDM/cryo/cryo_l{linac}b_main.edl")
            cryo_button.setToolTip(f"L{linac}B Cryo")
            magnet_button = PyDMRelatedDisplayButton(filename=f"$PYDM/mgnt/l{linac}b_main.ui")
            magnet_button.setToolTip(f"L{linac}B Magnets")
            vacuum_button = PyDMRelatedDisplayButton(filename=f"$PYDM/vac/l{linac}b_main.ui")
        ...
        global_cryo_button = PyDMRelatedDisplayButton(filename="$PYDM/cryo/global_main.ui")
