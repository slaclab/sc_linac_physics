...
This collection of scripts is to generate the appropriate data files, update, and run the field emission GUI. The purpose of the GUI is to aid in the visualization and characterization of field emission from linac cavities.
...
- **`plot_me.py`**: called by `field_emission_gui.py` for plotting. Contains the `fit_equation` method, to be further adjusted.


## Additional Scripts

- **`amp_vs_radiation_from_csv.py`**: takes start and end time data to find radiation during active cavity session. Necessary for gui_updater use. Contains the `align_pvs_to_common_time` method.


- **`gui_updater.py`**: worker for to update gui. Validator for single entry data and csv reader for multiple data entry.
...
- **Extra filter for measurement characteristics:** the addition of a way to further filter cryomodule data within the GUI. Columns have been included in csv reader and gui handler for additional filtering of CM data (recharacterization/multipacting/commissioning). They are also included as dataset attributes in the `.h5` file. Reflected in display (possibly) necessary.
